...
class CPCI7841ProtectedPart
{
 ...
 private:
  ...
  static const uint32_t PCI_MAX_BAR=6;//максимальное количество блоков в PCI
 ...
 public:
  ...
  void EnableTransmiter(uint32_t channel);//разрешить передачу данных
  void DisableTransmiter(uint32_t channel);//запретить передачу данных
  void DisableAllInterrupt(uint32_t channel);//запретить все прерывания
